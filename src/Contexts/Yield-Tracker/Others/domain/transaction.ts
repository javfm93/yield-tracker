...
export class Transaction extends HashEntity<TransactionProps> {
  private constructor(hash: string, props: TransactionProps) {
    super(hash, props);
  }

  public static create(hash: string, props: CreateTransactionProps) {
  ...
  isTransferOf(token?: Token | string): boolean {
    return typeof token === 'string' ? this.props.token.getSymbol() === token : this.props.token.isEqualTo(token);
  }
